...
let Artnet = function (config = {}) {
    ...
    this.set = function ({ universe = 0, channel = 1, value, pixelSize = 1, duplicateUniverse, callback = () => {} }) {
        ...
        let pixelLength = 512 - 512 % pixelSize;

        if (universe == 0 && Array.isArray(value) && value.length > pixelLength) {
            const chunkArray = (arr, size) =>
                arr.length > size
                    ? [arr.slice(0, size), ...chunkArray(arr.slice(size), size)]
                    : [arr];

            const values = chunkArray(value, pixelLength);

            for (let universe = 0; universe < values.length; universe++) {
                initData(universe);
                if (typeof duplicateUniverse == 'number') initData(universe + duplicateUniverse);

                dataChanged[universe] = dataChanged[universe] || 0;
                if (typeof duplicateUniverse == 'number') dataChanged[universe + duplicateUniverse] = dataChanged[universe + duplicateUniverse] || 0;
                ...
                for (let currentChannel = 0; currentChannel < value.length; currentChannel++) {
                    ...
                    if (typeof value[currentChannel] === 'number' && data[universe][index] !== value[currentChannel]) {
                        ...
                        if (typeof duplicateUniverse == 'number') data[universe + duplicateUniverse][index] = value[currentChannel];
                        if ((index + 1) > dataChanged[universe]) {
                            dataChanged[universe] = index + 1;
                            if (typeof duplicateUniverse == 'number') dataChanged[universe + duplicateUniverse] = index + 1;
                        }
                    }
                    if (dataChanged[universe]) {
                        that.send(universe, currentChannel == 0 ? callback : () => { });
                        if (typeof duplicateUniverse == 'number') that.send(universe + duplicateUniverse, () => { });
